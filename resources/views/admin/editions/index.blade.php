...
@section('maincontent')
    <x-pages.index titulo="Edições do sistema" subtitulo="Gerenciamento de edições"
        searchPlaceholder="Procurar por título ..." titleBtnPesquisar="Pesquisar" idBtnPesquisar="search-button"
        routeCreatenew="{{ route('admin.editions.create') }}" routeSearch="{{ route('admin.editions.index') }}"
        textCreateNew="Criar Nova Edição" :columns="$columns" :actions="$actions" :data="$formattedData" :paginator="$editions" />
@endsection
