...
{
    private const CACHE_KEY = 'ai.settings.resolved';
    ...
    public function update(array $data, User $user): AiSetting
    {
        return DB::transaction(function () use ($data, $user): AiSetting {
            ...
            $setting->save();
            $version = $setting->versions()->create(['provider' => $setting->provider, 'model' => $setting->model, 'technical_prompt' => $setting->technical_prompt, 'selection_prompt' => $setting->selection_prompt, 'technical_prompt_version' => $setting->technical_prompt_version, 'selection_prompt_version' => $setting->selection_prompt_version, 'knowledge_version' => $setting->knowledge_version, 'changed_by' => $user->id]);
            Cache::forget(self::CACHE_KEY);
            Cache::forever(self::CACHE_KEY, $this->fromModel($setting, $version));
    ...
    public function maskedKey(?AiSetting $setting = null): ?string
    {
        ...
        return $key ? 'Configurada ••••'.strtoupper(substr($key, -4)) : null;
    }
    ...
    public function forCorrelation(string $correlationId): AiSettingsData
    {
        ...
        return new AiSettingsData($current->id, $version->id, $version->provider, $version->model,
            $current->apiKey, $version->technical_prompt, $version->selection_prompt,
            ...
            $current->tries, $version->knowledge_version);
    }
